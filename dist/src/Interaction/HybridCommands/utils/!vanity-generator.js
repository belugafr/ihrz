...
function VerifyVanityCode(VanityCode) {
...
async function VanityCodeAlreadyExist(AllVanityGuild, code) {
    let _ = false;
    for (let guildId in AllVanityGuild) {
        if (AllVanityGuild[guildId]?.vanity === code)
            _ = true;
    }
    return _;
}
export default {
    run: async (client, interaction, lang, command, neededPerm, args) => {
        ...
        if (interaction instanceof ChatInputCommandInteraction) {
            var VanityCode = interaction.options.getString('code');
        }
        else {
            var VanityCode = client.method.string(args, 0);
        }
        ;
        ...
        ;
        let guildInvite = await interaction.guild.invites.create(interaction.channel, { temporary: false, reason: "iHorizon - VanityGenerator", maxAge: 0 });
        if (guildGet) {
