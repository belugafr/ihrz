...
async function generateBotHTML(client, data, bot, lang) {
...
async function buildEmbed(client, data, lang, interaction) {
...
export default {
    run: async (client, interaction, lang, command, neededPerm) => {
        ...
        for (let botId in data_2) {
            if (data_2[botId]) {
                const { embed, attachment } = await buildEmbed(client, data_2[botId], lang, interaction);
                embeds.push(embed);
        ...
        if (allData && allData[interaction.user.id]) {
            for (let botId in allData[interaction.user.id]) {
                const { embed, attachment } = await buildEmbed(client, allData[interaction.user.id][botId], lang, interaction);
                embeds.push(embed);
                attachments.push(attachment);
            }
        }
