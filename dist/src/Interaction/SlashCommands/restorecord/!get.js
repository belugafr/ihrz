...
*/
import { ActionRowBuilder, AttachmentBuilder, ButtonBuilder, ButtonStyle, EmbedBuilder, PermissionsBitField, } from 'discord.js';
...
export default {
    run: async (client, interaction, lang, command, neededPerm) => {
        ...
        const AllUsersData = await (client.db.table("RESTORECORD")).get("saved_users");
        if (!Data)
        ...
        const mainEmbed = new EmbedBuilder()
            ...
            .setFields({ name: lang.rc_get_mainEmbed_field1_name, value: Data.id || "", inline: true }, { name: lang.rc_get_mainEmbed_field2_name, value: interaction.guild.roles.cache.get(Data.data.config.roleId)?.toString() || Data.data.config.roleId, inline: true }, { name: lang.rc_get_mainEmbed_field3_name, value: `${lang.rc_get_mainEmbed_field3_value}\n\n${format(new Date(Data.data.config.createDate || 0), "MM/DD/YYYY HH:mm")}`, inline: false }, { name: lang.rc_get_mainEmbed_field4_name, value: String(Data.data.config.securityCodeUsed || 0), inline: true }, { name: lang.rc_get_mainEmbed_field5_name, value: (await client.users.fetch(Data.data.config.author.id)).toString() || lang.rc_get_unkwnon_user.replace("${Data.data.config.author.id}", Data.data.config.author.id), inline: true })
            .setFooter(footer);
        ...
        const localeData = Object.entries(members.reduce((acc, member) => {
        ...
        const updateComponents = () => [
            new ActionRowBuilder()
                ...
                .setDisabled(currentCategory !== 1 || currentPage === 0), new ButtonBuilder()
                .setStyle(ButtonStyle.Secondary)
            ...
            new ActionRowBuilder()
                ...
                .setDisabled(currentCategory === 0), new ButtonBuilder()
                .setStyle(ButtonStyle.Secondary)
