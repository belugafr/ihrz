...
export const command = {
    ...
    run: async (client, interaction) => {
        ...
        let embed = new EmbedBuilder()
            ...
            .setThumbnail(yes.tracks[0].info.artworkUrl);
        ...
        ;
        await client.db.push(`${player.guildId}.MUSIC_HISTORY.buffer`, `[${(new Date()).toLocaleString('fr-FR', { timeZone: 'Europe/Paris' })}: PLAYED]: { ${yes.tracks[0]?.requester} - ${yes.tracks[0].info.title} | ${yes.tracks[0].info.uri} } by ${yes.tracks[0]?.requester}`);
        await client.db.push(`${player.guildId}.MUSIC_HISTORY.embed`, `${time(new Date(), 'R')}: ${yes.tracks[0]?.requester} - ${yes.tracks[0].info.title} | ${yes.tracks[0].info.uri} by ${yes.tracks[0]?.requester}`);
