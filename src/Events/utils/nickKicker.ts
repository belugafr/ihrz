    ...
    run: async (client: Client, member: GuildMember) => {
        ...
        try {
            ...
            if (baseData?.enabled && baseData.words.some(word => member.user.username.toLowerCase().includes(word.toLowerCase()))) {
                member.send({ content: "In the guild `" + member.guild.name + "`, your username contains a blacklisted word. You have been kicked from the guild." })
                    .catch(() => { })
                    .then(() => { });
                member.kick(`Blacklisted word in username`)
                    .catch(() => { })
                    .then(() => { });
            }
