...
export default {
    run: async (client: Client, interaction: ChatInputCommandInteraction<"cached"> | Message, lang: LanguageData, command: SubCommandArgumentValue, neededPerm?: number, args?: string[]) => {
        ...
        let baseData = await client.db.get(`${interaction.guildId}.UTILS.LEASH_CONFIG`) as DatabaseStructure.LeashConfig;
        ...
        if (!isInVoiceChannel(user) || isInVoiceChannel(interaction.member)) {
            await client.method.interactionSend(interaction, { content: `${client.iHorizon_Emojis.icon.No_Logo} | The member you want to leash (or yourself) is not in voice channel!` });
            return;
        }

        await client.db.push(`${interaction.guildId}.UTILS.LEASH`, { dom: interaction.member.user.id, sub: user.id });

        await client.method.interactionSend(interaction, { content: `${client.iHorizon_Emojis.icon.Yes_Logo} | You have sucessfuly leashed the user in this guild :smirk:` })
    },
};